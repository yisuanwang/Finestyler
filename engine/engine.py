...
def inference(test_loader, model, args):
    iou_list = []
    for img,param in test_loader:
        print("777",img.shape)
        print(type(img))
        print(param['ori_img'][0].cpu().numpy())
        print(param['ori_img'][0].cpu().numpy().shape)
        print(type(param['ori_img'][0].cpu().numpy()))
        print("###")
        exit()
...
@torch.no_grad()
def predict(img,sent, model, args):
    model.eval()
    time.sleep(2)
    ...
    pred = pred.cpu().numpy()
    print(pred.shape)
    pred = cv2.warpAffine(pred, mat_inv, (ori_w,ori_h),
                            flags=cv2.INTER_CUBIC,
                            borderValue=0.)
    print(pred.shape)
    pred = np.array(pred > 0.35)
