...
class UpBlock(nn.Module):
    ...
    def forward(self, input, FB_in):
        ...
        # print("out_temp size:", out_temp.size())
        # print("FB_in size:", FB_in.size())

        out_temp_resized = F.interpolate(out_temp, size=(FB_in.size(2), FB_in.size(3)), mode='bilinear', align_corners=True)
        # print('input size=',input.shape)
        # print('out_tmp size=',out_temp.shape)
        # print('out_temp_resized size=',out_temp.shape)
        # print('FB_in size=',out_temp.shape)

        out_temp = torch.cat([out_temp_resized, FB_in], dim=1)
